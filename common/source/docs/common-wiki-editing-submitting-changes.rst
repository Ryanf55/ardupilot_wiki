...
#. "Squash" your commits. See :ref:`git-interactive-rebase`. There should be only one commit for changes related to a given topical change, even if multiple pages with changes are involved. Commits should be separated per site, such as one commit for Copter and one commit for Dev if the changes affect both.
